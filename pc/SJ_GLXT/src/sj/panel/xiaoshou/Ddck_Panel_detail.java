...
import sj.panel.xiaoshou.Tjdd_Panel_detail.AddDetail;
import sj.panel.xiaoshou.Tjdd_Panel_detail.Have_kc_dd;
...
public class Ddck_Panel_detail extends JPanel
{
	...
	public JScrollPane createDetailPane()
	{
		...
		String[] tableHeads = new String[]{"编号" , "商品名称" , "商品数量" , "单位" 
				, "商品单价"  , "备注信息", "总价/元" , "实付金额" , "欠付金额" , "库存" , "安装工人" , "安装费用" };
			...
			public void tableChanged(TableModelEvent e) 
			{
				if(e.getType() == TableModelEvent.UPDATE)
				{
					...
					for(int i=0;i<v.size();i++)
					{
						...
						{			
							list.add(v1);
							DB_Opreat.update_dd_detail_1( list, String.valueOf(jh_index));
							setTableDetail(DB_Opreat.get_item_dd_detail(jh_index) , jh_index);
	...
	class AddDetail extends JPanel
	{
		...
		AddDetail()
		{
			...
			gridBagConstraints_6.insets = new Insets(5, 3, 0, 3);
			...
			//安装工人
			this.setupComponet(new JLabel("安装工人"), 2, 2, 1, 0, true);
			this.gongren = new JTextField();
			this.setupComponet(this.gongren, 3, 2, 1, 120, true);
			
			//安装费用
			this.setupComponet(new JLabel("安装费"), 4, 2, 1, 0, true);
			this.anzhuangfei = new JTextField();
			this.setupComponet(this.anzhuangfei, 5, 2, 1, 120, true);
			
			//添加按钮
			this.add = new JButton("添加商品记录");
			this.setupComponet(this.add, 6, 2, 1, 0, true);
			this.add.addActionListener(new AddButtonActionListener());
			//重置按钮
			this.reset = new JButton("重置");
			this.setupComponet(this.reset, 7, 2, 1, 0, true);
			this.reset.addActionListener(new RestButtonActionListener());
			
			this.shuchu = new JButton("输出");
			this.setupComponet(this.shuchu, 8, 2, 1, 0, true);
			...
			this.setupComponet(shifu, 2, 4, 2, 0, true);
			
			//欠付显示
			qianfu = new JTextField();
			qianfu.setEditable(false);
			qianfu.setFont(new Font("隶书" , Font.BOLD, 14));
			this.setupComponet(qianfu, 4, 4, 3, 0, true);
			
			//安装费用
			anzhuang = new JTextField();
			anzhuang.setEditable(false);
			anzhuang.setFont(new Font("隶书" , Font.BOLD, 14));
			this.setupComponet(anzhuang, 7, 4, 3, 0, true);
		...
		private final class AddButtonActionListener implements ActionListener
		{
			...
			{		
				...
				item.add("---");
				item.add(gongren.getText());
				double azf = Double.parseDouble(anzhuangfei.getText());
				item.add(azf);
				dftm.addRow(item);
				sum = sum+je;
				sum1 = sum1+sf;
	...
	public class Have_kc_dd extends JPanel
	{
		JPanel ha = this;
		JComboBox spmc_cb;
		JTextField Id_t;
	...
	{		
		...
		for(int i =0; i<list.size();i++)
		{
			...
			sum2 = sum-sum1;
			sum3 = sum3+Calculat.Result1(item.get(11).toString());
			dftm.addRow(item);
		}
		updateZJ();
	}
	...
	private void updateZJ()
	{
		...
		this.anzhuang.setText("安装合计："+Calculat.Result(sum3));
	}	
	...
	{
		return DB_Opreat.insert_dd_detail(dftm.getDataVector(), id_index);
	}
	...
	{
		DB_Opreat.remove_item_sp(id , "t_dingdan_detail");
	}
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (type == JOptionPane.YES_OPTION)
			{
				...
				{	
					...
					sum2 = sum-sum1;
					sum3 = sum3-Double.parseDouble(table.getValueAt(id, 11).toString());
					updateZJ();
					dftm.removeRow(id);
					count--;
				}
